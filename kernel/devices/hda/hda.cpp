...
static void queue_enqueue(queue_t queue, void *data) {
    ...
    queue_node_t node = _Malloc1(sizeof(*node));
    node->data        = data;
    node->next        = null;
...
void hda_init(PCI::PCIDeviceHeader *PciBaseAddr) {
    PrintMsg("HD Audio Driver Initializing...");
    ...
    */
    timerctl.count = PIT::TimeSinceBootMS();
    PCI::enable_interrupt(PciBaseAddr);
    PCI::enable_bus_mastering(PciBaseAddr);
    PCI::enable_mem_space(PciBaseAddr);
    hda_buffer_ptr = _Malloc1(4096 * 2);
    info("hda card found at bus %d slot %d func %d", hda_bus, hda_slot, hda_func);
    hda_base = ((PCIHeader0*)PciBaseAddr)->BAR0;
    ...
    output_buffer = (u32*)GlobalAllocator->RequestPage();
    ...
    PrintfMsg("> HD Audio IRQ: %d", irqId);
    {
        IRQHandlerCallbackFuncs[irqId] = (void*)&hda_interrupt_handler;
        IRQHandlerCallbackHelpers[irqId] = (void*)0;
    }
    ...
    corb = (u32*)GlobalAllocator->RequestPage();
    ...
    rirb = (u32*)GlobalAllocator->RequestPage();
    asm_set32(hda_base + 0x50, (u32)rirb);
    asm_set32(hda_base + 0x54, 0);

    rirb_size = asm_get8(hda_base + 0x5e) >> 4 & 0x0f;
    if (rirb_size & 0b0001) {
        rirb_entry_count = 2;
...
void hda_interrupt_handler() {
    ...
    //send_eoi(0x0b);
    asm_out8(PIC1_OCW2, 0x60 | (0x0b - 8));
